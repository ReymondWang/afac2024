...
from typing import  Any, Union, Optional, Dict #Annotated,
import numpy as np
...
class Seq2SeqTrainer(_Seq2SeqTrainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[str, torch.Tensor]]) -> torch.Tensor:
    ...
    def prediction_step(
            ...
            ignore_keys=None,
            **gen_kwargs,
    ) -> tuple[Union[float, None], Union[torch.Tensor, None], Union[torch.Tensor, None]]:
...
def main(
        ...
        # ),
    data_dir: Union[str, typer.Argument(help='')],
    model_dir: Union[
        str,
        typer.Argument(
            help='A string that specifies the model id of a pretrained model configuration hosted on huggingface.co, or a path to a directory containing a model configuration file.'
    ...
    config_file: Union[str, typer.Argument(help='')],
    auto_resume_from_checkpoint: Optional[str] = typer.Argument(
        default='',
        help='If entered as yes, automatically use the latest save checkpoint. If it is a numerical example 12 15, use the corresponding save checkpoint. If the input is no, restart training'
    )
):
