...
def get_prompt(query,type_,glm3_output,stock_names,fund_names):
    '''
    ...
    if type_ & STOCK_INQUIRY:
        products = difflib.get_close_matches(query,stock_names,n=50,cutoff=0.0001)
        query += '\n    query中提到的产品标准名可能是：' + '、'.join(products)
    elif type_ & FUND_INQUIRY:
        products = difflib.get_close_matches(query,fund_names,n=50,cutoff=0.0001)
        query += '\n    query中提到的产品标准名可能是：' + '、'.join(products)
    res = {}
    res['normal_prompt'] = PROMPTS_MAP[type_][1].replace('<QUERY>',query)
    res['correct_prompt'] = PROMPTS_MAP[type_][2].replace('<QUERY>',query)\
        .replace('<GLM3_ANSWER>',f'基础模型答案是：{glm3_output}')
    return res
