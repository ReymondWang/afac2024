import datasets
import torch
from tools.common_utils import TemporarilySeededRandom
...
def get_dataset(data_dir: str, data_files: str, split: str, split_ratio: float = 0.05, silent: bool = False) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    r"""
        将csv数据转换成dpo训练需要的数据集，并且根据split进行分割。
        test、eval的分割比例为0.05
        
        Args:
            params (`data_dir`): csv文件存放的路径
            params (`data_files`): csv文件名
            params (`split`): 分割数据集，取值为train、test、eval
            params (`split_ratio`): 分割比例
            params (`silent`): 是否静默处理
    """
    dataset = datasets.load_dataset(path=data_dir, data_files=data_files, split="train")
    
    dataset = dataset.shuffle(seed=42)
    dataset = dataset.select(range(int(len(dataset) * split_ratio))) if split == 'test' or split == 'eval' else dataset.select(range(int(len(dataset) * split_ratio), len(dataset)))
    
    data = defaultdict(lambda: defaultdict(list))
    for row in tqdm.tqdm(dataset, desc='Processing ' + split + " data", disable=silent):
        prompt = '\n\nHuman: ' + row['query'] + '\n\nAssistant:'
        label = row["label"]
        predict = row["predict"]
...
def get_batch_iterator(data_dir: str, 
                       data_files: str,
                       tokenizer,
                       split: str = 'train',
                       batch_size: int = 1,
    ...
    with TemporarilySeededRandom(seed):
        ...
        for prompt, data in get_dataset(data_dir = data_dir, data_files=data_files, split = split, silent = silent).items():
...
if __name__ == "__main__":
    data = get_dataset(data_dir="../data", data_files="train_err.csv", split="test")
    print(data)